...
def pierre_AC3(csp, queue=None, removals=None, arc_heuristic=dom_j_up):
    """[Figure 6.3]"""
    if queue is None:
        queue = {(Xi, Xk) for Xi in csp.variables for Xk in csp.neighbors[Xi]}
    csp.support_pruning()
    ...
    while queue:
        ...
        if revised:
            ...
            for Xk in csp.neighbors[Xi]:
                if Xk != Xj:
                    queue.add((Xk, Xi))
    return True, checks  # CSP is satisfiab
def pierre_revise(csp, Xi, Xj, removals, checks=0):
    ...
    for x in csp.curr_domains[Xi][:]:
        ...
        for y in csp.curr_domains[Xj]:
            if csp.constraints(Xi, csp.curr_domains[Xi][x], Xj, csp.curr_domains[Xj][y]):
                conflict = False
            checks += 1
            if not conflict:
                break
        if conflict:
            csp.prune(Xi, csp.curr_domains[Xi][x], removals)
            revised = True
